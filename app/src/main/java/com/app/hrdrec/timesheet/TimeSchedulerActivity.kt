...
@AndroidEntryPoint
class TimeSchedulerActivity : Fragment(R.layout.activity_time_scheduler),
    CommonInterface, View.OnClickListener, TimeSchudeListAdapter.FeatureCallBack {
    ...
    private fun setObserver() {
        ...
        timeSheetViewModel.addClientsResponse.observe(viewLifecycleOwner) {
            it.message?.let { it1 ->
                CommonMethods.showToast(requireContext(), it1)
                CommonMethods.hideLoader()
            }
        }
    }
    ...
    override fun onClick(v: View) {
        when (v.id) {
            ...
            R.id.btnSubmit -> {
                if (binding.txtFrom.text.toString().isNotEmpty() && binding.txtTo.text.toString().isNotEmpty())
                    timeSheetViewModel.getTimesheetschedulerDates(
                        binding.txtFrom.text.toString(),
