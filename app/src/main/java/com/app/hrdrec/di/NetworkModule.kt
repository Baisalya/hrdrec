...
//  fun provideBaseURL() = "http://192.168.88.104:8095/hrdome/"
    ...
    fun provideHeaderInterceptor(@ApplicationContext context: Context): Interceptor {
        return Interceptor { chain ->
            ...
            var token=""
            if(SharedPrefUtils(context).getUserInfo()!=null)
                token= SharedPrefUtils(context).getUserInfo()!!.token.toString()
            Log.e("Token","ssd "+SharedPrefUtils(context).getSavedBoolean(ISLOGINAPI))
            val request: Request = if (!token.isNullOrEmpty()&&!SharedPrefUtils(context).getSavedBoolean(ISLOGINAPI)) {
